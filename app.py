...
# ─────────────────────────────────────────────
DB_PATH = "cattle_breed_db.sqlite3"

def init_db():
    """Initialize SQLite database with breeds schema if not exists."""
    if not os.path.exists(DB_PATH):
        try:
            ...
            CREATE TABLE IF NOT EXISTS breeds (
                ...
                milk_productivity TEXT NOT NULL,
...
def get_db_connection():
    """Return a live SQLite connection, or None on failure."""
    try:
        conn = sqlite3.connect(DB_PATH)
...
def breeds():
    ...
    if conn:
        try:
            ...
            cattle_list = [dict(row) for row in cursor.fetchall()]
            cursor.execute(
                "SELECT * FROM breeds WHERE animal_type='buffalo' ORDER BY breed_name"
            )
            buffalo_list = [dict(row) for row in cursor.fetchall()]
            conn.close()
            
            if not cattle_list and not buffalo_list:
                raise Exception("Empty database tables, triggering fallback")
